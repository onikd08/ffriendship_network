...
def read_friendship_network(filename):
    ...
    :return: dict(list(str)) network: The dictionary containing the friendship
                                      network information
             or
...
def user_interface(network):
    ...
    :param dict(list(str)) network: The dictionary containing the information
                                    about who is friends with whom.
    :return: None
    """
...
def add_friendship(network, name1, name2):
    ...
    :param dict(list(str)) network: Dict containing the friendship network.
                                    This parameter will be modified to include
                                    the new two way friendship.
    ...
    if name2 not in network[key]:
        network[key].append(name2)    # <name2> is <name1>'s friend

    key = name2
    network.setdefault(key,[])
    if name1 not in network[key]:
        network[key].append(name1)    # <name2> is <name1>'s friend
...
def print_command(network, namelist):
    ...
    :param dict(list(str)) network: Dict that contains friendship network
                                    to be printed.
    ...
    for name, friends in sorted(network.items()):
        print(name)
        for friends_name in sorted(friends):
            print("-", friends_name)
...
def add_command(network, namelist):
    ...
    :param dict(list(str)) network: The friendship network dictionary which
                                    will be modified as a result of the call
                                    to this function.
...
def friends_command(network, namelist):
    ...
    :param dict(list(str)) network: Dictionary containing the friendship
                                   information.
    ...
    # described in the project instructions.
        ...
        friendlist = network.get(name)
        for friend in sorted(friendlist):
            print(friend)
...
def common_friends_command(network, namelist):
    ...
    :param dict(list(str)) network: Data structure containing the friendship
                                   information.
    ...
    friends_of_name1 = network.get(name1)
    friends_of_name2 = network.get(name2)
    common_freinds = []

    for name1_friends in friends_of_name1:
        for name2_friends in friends_of_name2:
            if name1_friends == name2_friends:
                common_freinds.append(name1_friends)

    #  If <name1> and <name2> do not have common friends,
    #  print instead an error message:

    if len(common_freinds) == 0:
        print(f"{name1} and {name2} have no common friends.")

    else:
        for names in sorted(common_freinds):
            print(names)
